...
当前仓库已完成阶段五：在阶段四多主体任务和真实数据预测基础上，增加了多模型候选、双目标模型选择、多窗口稳健性验证、交易成本敏感性分析和可追溯的选择理由。
...
34 项测试覆盖早期阶段回归、四类市场、数据治理、真实数据预测、时间可用性、多主体清单、重试恢复、失败隔离、调度间隔、行情交叉校验、版本注册、五模型选择、稳健性窗口、成本敏感性和 Excel 契约。`run_acceptance.py` 使用固定样例执行完整批量入口、单股报告和汇总报告验收。阶段实施方式与实际验证记录见 [工作计划.md](工作计划.md)。
